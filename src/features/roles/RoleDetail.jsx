...
import { useRole, useUpdateRole, useUpdateRolePermissions } from "../../hooks/useRoles";
...
export default function RoleDetail() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updatePerms.mutateAsync({ id, permissionIds: selected });
  ...
  return (
    <div className="flex flex-col h-full">
      <ContentHeader
        title={t("roleDetail", { defaultValue: "Role Detail" })}
        actions={
          <>
            <Button variant="outline" onClick={() => navigate(-1)}>{t("cancel")}</Button>
            <Button onClick={handleSave} disabled={updateRole.isLoading || updatePerms.isLoading}>{t("save")}</Button>
          </>
        }
      />
